            ...
            if (isset($_GET['token'])) {
                ...
                if ($result_select->num_rows > 0) {
                    ...
                    if($used == 0){
                                    ...
                                    if ($attendance === null) {
                                        ...
                                        if ($conn->query($sql_update) === TRUE) {
                                            ...
                                            if ($conn->affected_rows > 0) {
                                                ...
                                                $mensaje_respuesta = "Registro de asistencia exitoso.";
                                            } 
                                        } else {
                                            // Si ocurrió un error al actualizar, devuelve el mensaje de error de MySQL
                                            $mensaje_respuesta = "Error al registrar la asistencia.";
                                        }
                                    } elseif($attendance == 1){
                                        // Si no se actualizó ningún registro (ya se había registrado la asistencia previamente), muestra un mensaje informativo
                                        $mensaje_respuesta = "La asistencia ya ha sido registrada para este alumno y esta materia hoy.";
                                    }
                                    else{
                                        $mensaje_respuesta = "La clase ya ha sido cerrada.";
                                    }
                                } else {
                                    $mensaje_respuesta = "Error: No se pudo verificar la asistencia.";
                                }
                            } else {
                                $mensaje_respuesta = "Error: No se encontró un alumno asociado al usuario.";
                            }
                        } else {
                            $mensaje_respuesta = "Error: La materia asociada al token no coincide con las materias que imparte el profesor.";
                        }
                        ...
                        if($used == 1){
                        $mensaje_respuesta = "Error: El código ya fue usado";// manda el mensaje si el código QR ya fue usado
                        }
                    }
                } else {
                    $mensaje_respuesta = "Error: No se encontró ningún token asociado.";
                }
            } else {
                // Si no se proporciona un token, se devuelve un error
                $mensaje_respuesta = "Error: No se proporcionó un token.";
            }
        } else {
...
// Redirecciona a send_email.php con el token como parámetro
header("Location: send_email.php?token=$token");
exit;
?>
