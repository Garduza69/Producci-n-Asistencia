...
JOIN 
    horarios h ON m.grupo_id = h.grupo_id  -- Unir con la tabla de horarios
JOIN 
    materias ma ON m.materia_id = ma.materia_id 
WHERE 
    h.profesor_id = ?  -- Filtrar por el ID del profesor
    AND g.vigenciaSem = 1  -- Asegura que los grupos estén vigentes
GROUP BY 
    ma.nombre 
ORDER BY 
    ma.nombre;";
